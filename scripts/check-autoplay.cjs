// Ambient music autoplay regression: node scripts/check-autoplay.cjs
// A: browser permits autoplay -> music starts with zero interaction.
// B: strict default policy -> first click still unlocks it.
...
const launch={executablePath:process.env.BROWSER_EXECUTABLE || '/var/lib/flatpak/app/com.brave.Browser/current/active/files/brave/brave',headless:true,args:['--no-sandbox','--disable-dev-shm-usage']};
const audible=()=>window.pix3lwareAudio && window.pix3lwareAudio.state==='running' && window.pix3lwareAudio.gain>0.01;
(async()=>{
 const permissive=await chromium.launch({...launch,args:[...launch.args,'--autoplay-policy=no-user-gesture-required']});
 try {
 ...
 const strict=await chromium.launch(launch);
 try {
