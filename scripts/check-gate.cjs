 ...
 try {
  for(const gesture of ['click','key']) {
   ...
   try {
    ...
    assert.equal(look.anim,'none','no CSS flicker; Web Animations API drives the blink');
    assert.ok(await page.evaluate(()=>document.querySelector('.loader-enter').getAnimations().some(a=>a.playState==='running')),'WAAPI blink is running');
